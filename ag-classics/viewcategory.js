...
/* ======================================================
   ...
   ✅ If slug is PARENT → includes all child category products
   ✅ Only agclassics categories
====================================================== */
router.get("/:slug/products", (req, res) => {
  ...
  db.query(
    `SELECT id, parent_id FROM categories WHERE slug = ? AND imprint = 'agclassics' LIMIT 1`,
    [slug],
    (err, cats) => {
...
/* ======================================================
   CATEGORY LIST WITH PRODUCT COUNT — agclassics only
====================================================== */
  ...
  db.query(sql, (err, rows) => {
    ...
    const result = rows.map(r => ({
      ...
      product_count: r.parent_id === null
        ? (childMap[r.id] || 0) + r.product_count
        : r.product_count,
    }));
    ...
    (err, cats) => {
      ...
      db.query(
        ...
        (err2, catRows) => {
          ...
          const sql = `
            SELECT
              a.id, a.name, a.slug, a.profile_image,
              COUNT(DISTINCT p.id) AS product_count
            FROM authors a
    ...
    (err, cats) => {
      ...
      db.query(
        ...
        (err2, catRows) => {
          ...
          const sql = `
            SELECT a.id, a.name, a.slug, a.profile_image,
              COUNT(DISTINCT p.id) AS product_count
            FROM authors a
            JOIN product_authors pa ON pa.author_id = a.id
            JOIN products p ON p.id = pa.product_id
