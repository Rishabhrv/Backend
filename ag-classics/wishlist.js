...
router.get("/ids", auth, (req, res) => {
  db.query(
    `SELECT w.product_id
     ...
     JOIN categories c ON c.id = pc.category_id
     WHERE w.user_id = ? AND c.imprint = 'agclassics'`,
    [req.user.id],
    (err, rows) => {
      if (err) return res.status(500).json(err);
      res.json({ success: true, ids: rows.map((r) => r.product_id) });
    }
  );
});
      ...
      db.query(
        ...
        (err) => {
          ...
          db.query(
            ...
            (err, rows) => {
              if (err) return res.status(500).json(err);
              res.json({ success: true, message: "Added to wishlist", wishlist_count: rows[0].count });
            }
          );
        }
      );
